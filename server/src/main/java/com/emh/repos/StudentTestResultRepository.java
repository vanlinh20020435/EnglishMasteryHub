...
import com.emh.payload.response.IStudentSummaryResponse;
...
public interface StudentTestResultRepository extends JpaRepository<StudentTestResult, Integer>, JpaSpecificationExecutor<StudentTestResult>
{
	...
	@Query(value = "SELECT s.student_id studentId, str.id testResultId\n" +
			"FROM student s\n" +
			"LEFT JOIN (\n" +
			"        SELECT id, score, student_id, class_id, ROW_NUMBER() OVER (PARTITION BY student_id ORDER BY id DESC) AS newest\n" +
			...
			"     AND str.newest = 1\n" +
			"WHERE s.class_id = ?1\n", nativeQuery = true)
	List<IStudentSummaryResponse> statistic(int classId, int testId);

	@Query(value = "SELECT s.student_id studentId, s.name studentName, str.score, str.id testResultId\n" +
			"FROM student s\n" +
			"LEFT JOIN (\n" +
			"        SELECT id, score, student_id, class_id\n" +
			"        FROM student_test_result\n" +
			"        WHERE class_id = ?1 AND test_code_id = ?2\n" +
			"        ORDER BY id DESC\n" +
			"    ) str \n" +
	...
	List<IStudentSummaryResponse> statistic(int classId, int testId, int studentId);
}
