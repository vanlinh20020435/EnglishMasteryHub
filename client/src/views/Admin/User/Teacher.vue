...
import { getTeachers, searchTeachers, createAdmin, editAdmin } from '@/services'
...
export default {
    ...
    data() {
        return {
            ...
            formValid: false,
            headers: [
                { title: 'Teacher ID', align: 'center', key: 'teacherId', sortable: false },
    ...
    methods: {
        ...
        fetchFilter() {
            ...
            this._timerId = setTimeout(async () => {
                this.isLoadingData = true
                const res = await searchTeachers(this.authentication?.accessToken?.token, this.filter)
                if (res.success) {
                    this.data = res.data
                }
                this.isLoadingData = false
        ...
        async editItem() {
            ...
            const res = await editAdmin(this.formItem.teacherId, this.authentication?.accessToken?.token, payload)
            this.isLoadingForm = false
            if (res.success) {
                console.log(res);
                this.isOpenForm = false
