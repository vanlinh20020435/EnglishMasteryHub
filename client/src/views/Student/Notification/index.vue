<template>
  ...
  <v-list v-else-if="examNotis.length" style="padding-top: 0; padding-bottom: 0">
    <v-card-title v-if="examNotis.length" style="padding-top: 0; padding-bottom: 0; font-size: 1rem">Bài kiểm tra đang
      diễn ra</v-card-title>
    <v-hover v-for="noti in examNotis" v-slot="{ isHovering, props }">
      <v-card @click="$router.push(`/student/test/${noti.testId}`)" :class="{ 'on-hover': isHovering }"
