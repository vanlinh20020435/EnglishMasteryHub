<template>
  ...
  <v-list v-else-if="tests.length" style="padding-top: 0; padding-bottom: 0">
    <v-hover v-for="test in tests" v-slot="{ isHovering, props }">
      <v-card
        :disabled="!validTest(test.startDate, test.endDate)"
        @click="$router.push(`/student/test/${test.testId}`)"
        ...
        <v-list-item
          ...
          <template v-slot:append>
            <div
              ...
              ">
              <v-list-item-title
                >Thời gian làm bài: {{ test.time }} phút</v-list-item-title
              >
              <v-list-item-subtitle
                >Tổng số câu hỏi:
                {{ test.totalQuestions }} câu</v-list-item-subtitle
              >
            </div>
          </template>
        </v-list-item>
  ...
  <v-card v-else class="d-flex justify-center"
    >Bạn không có bài kiểm tra nào</v-card
  >
</template>
