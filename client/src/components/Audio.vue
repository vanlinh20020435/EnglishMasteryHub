<template>
    <v-card style="text-align: center" :flat="flat == undefined || flat == false ? false : true">
        <v-card-text class="d-flex">
            ...
            <div class="d-flex" style="flex: 7; margin-top: 8px;">
                <v-slider :track-size="1" v-model="percentage" @update:modelValue="setPosition"></v-slider>
                <p style="margin-top: 8px;">{{ currentTime }} / {{ duration }}</p>
            </div>
            <v-slider style="flex: 2; margin-top: 8px;" :track-size="1" v-model="playerVolume"
                :prepend-icon="volumeHighIcon" max="1" step="0.01" min="0"></v-slider>
        </v-card-text>
